...
def test_split_data(sample_data):
    """データ分割のテスト"""
    # 基本的な分割のテスト
    train_val, test = split_data(sample_data, test_size=0.2)
...
def test_preprocessor_with_unknown_categories(sample_data):
    ...
    assert unknown_category_value == -1, (
    ...
    assert all(val != -1 for val in known_category_values), (
    ...
    for feature in CATEGORICAL_FEATURES:
        if feature != "BROWSER":
            ...
            assert all(val != -1 for val in X_test_transformed[:, feature_idx]), (
                f"{feature}の値が不正にエンコードされています"
            )


@pytest.mark.parametrize("test_size", [0.1, 0.2, 0.3])
def test_split_data_different_sizes(sample_data, test_size):
    """異なる分割サイズでのテスト"""
    _, test = split_data(sample_data, test_size=test_size)
